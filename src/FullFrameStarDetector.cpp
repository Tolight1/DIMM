                                                          ...
                                                          double* otsuThresholdValue)
{
    ...
    if (detectionImage.empty() ||
        (detectionImage.depth() != CV_8U &&
         detectionImage.depth() != CV_16U &&
         detectionImage.depth() != CV_32F &&
         detectionImage.depth() != CV_64F)) {
        return candidates;
    }
    ...
    const int componentCount =
        cv::connectedComponentsWithStats(binary,
                                         ...
                                         stats,
                                         centroids,
                                         ConnectedDomain::sanitizeConnectivity(config.connectivity),
                                         CV_32S);
    ...
    for (int y = 0; y < labels.rows; ++y) {
        ...
        for (int x = 0; x < labels.cols; ++x) {
            ...
            if (label > 0 && label < componentCount) {
                const double value = rawPixelValueAt(detectionImage, y, x);
    ...
    for (int label = 1; label < componentCount; ++label) {
        ...
        const int minimumComponentArea =
            std::max(ConnectedDomain::kMinimumComponentArea, config.minArea);
        if (area < minimumComponentArea || area > config.maxArea) {
            continue;
        }
                                       ...
                                       double* peakValue)
{
    ...
    double maxValue = 0.0;
    cv::Point maxLoc;
    cv::minMaxLoc(detectionImage, &minValue, &maxValue, nullptr, &maxLoc);
    if (peakValue) {
        *peakValue = maxValue;
    }

    const InitialStarDetectionConfig config = currentInitialStarDetectionConfig();
    const double threshold = std::max(mean[0] + config.sigmaThreshold * stddev[0],
    ...
    const int minimumComponentArea =
        std::max(ConnectedDomain::kMinimumComponentArea, config.minArea);
    if (weightSum <= 0.0 || supportCount < minimumComponentArea || supportCount > config.maxArea) {
        return false;
    }
...
bool detectInitialStarCentroid(const cv::Mat& grayscale, QPointF* centroid, double* peakValue)
{
    ...
    const cv::Mat& nativeImage = grayscale;
    if (nativeImage.empty()) {
        return false;
    }

    cv::Scalar mean;
    cv::Scalar stddev;
    cv::meanStdDev(nativeImage, mean, stddev);

    double minValue = 0.0;
    double maxValue = 0.0;
    cv::Point maxLoc;
    cv::minMaxLoc(nativeImage, &minValue, &maxValue, nullptr, &maxLoc);
    if (peakValue) {
        *peakValue = maxValue;
    }
    ...
    if (maxLoc.x < 8 || maxLoc.y < 8 ||
        maxLoc.x >= nativeImage.cols - 8 || maxLoc.y >= nativeImage.rows - 8) {
        return false;
    }

    const int x0 = std::max(0, maxLoc.x - 8);
    const int y0 = std::max(0, maxLoc.y - 8);
    const int x1 = std::min(nativeImage.cols - 1, maxLoc.x + 8);
    const int y1 = std::min(nativeImage.rows - 1, maxLoc.y + 8);
    double weightedX = 0.0;
    double weightedY = 0.0;
    double weightSum = 0.0;
    ...
    for (int y = y0; y <= y1; ++y) {
        for (int x = x0; x <= x1; ++x) {
            const double value = rawPixelValueAt(nativeImage, y, x);
...
bool detectInitialStarCentroidFast(const cv::Mat& grayscale, QPointF* centroid, double* peakValue)
{
    ...
    if (maxLoc.x < kSearchRadius || maxLoc.y < kSearchRadius ||
        maxLoc.x >= nativeImage.cols - kSearchRadius ||
        maxLoc.y >= nativeImage.rows - kSearchRadius) {
        return false;
    }

    const int x0 = std::max(0, maxLoc.x - kSearchRadius);
    const int y0 = std::max(0, maxLoc.y - kSearchRadius);
    const int x1 = std::min(nativeImage.cols - 1, maxLoc.x + kSearchRadius);
    const int y1 = std::min(nativeImage.rows - 1, maxLoc.y + kSearchRadius);
    ...
    // Live full-frame seeding keeps its local contrast gate.
