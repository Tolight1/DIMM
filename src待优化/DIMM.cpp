...
bool SettingsDialog::applySettings()
{
    ...
    if (onApplyOptics) {
        onApplyOptics(diameter, baseline, baselineAngle, focal, zenithAngle, wavelength, pixelSize);
    }
    if (onApplyAlignment) {
    ...
    if (onApplyPolarisSolver) {
        onApplyPolarisSolver(alignmentAutoSolveCheck ? alignmentAutoSolveCheck->isChecked() : true,
...
{
    const auto action = AlignmentFlowCoordinator::actionForToggle(
        m_captureState == CaptureState::Alignment);
    if (action == AlignmentFlowCoordinator::ToggleAction::Stop) {
        stopAlignmentMode();
        return;
    }
...
{
    const AlignmentFlowCoordinator::StartReadiness readiness =
        AlignmentFlowCoordinator::validateStartReadiness(
...
void DIMM::showAlignmentModeStarted()
{
    ...
    setAlignmentSolveLabel(0, AlignmentFlowCoordinator::waitingLabelText(), UiStatusLevel::Info);
    setAlignmentSolveLabel(1, AlignmentFlowCoordinator::waitingLabelText(), UiStatusLevel::Info);
    setStatusMessage(AlignmentFlowCoordinator::startedStatusText(m_alignmentPreviewRateHz),
                     UiStatusLevel::Info);
}
...
void DIMM::showAlignmentModeStopped()
{
    ...
    setStatusMessage(AlignmentFlowCoordinator::stoppedStatusText(), UiStatusLevel::Warning);
}
...
void DIMM::resetAlignmentRuntimeForStart()
{
    ...
    if (m_polarisSolverController) {
        m_polarisSolverController->cancelAll(m_alignmentSolveGeneration);
    }
    auto& runtime = m_liveRuntime;
    for (int i = 0; i < 2; ++i) {
        AlignmentRuntimeCoordinator::LiveRuntimeAccess access;
        ...
        AlignmentRuntimeCoordinator::resetCameraForStart(access,
                                                         &m_alignmentLastFrame[i],
                                                         &m_alignmentLastFrameId[i],
                                                         &m_alignmentSolveResult[i],
                                                         &m_alignmentSolveRuntime[i],
                                                         m_alignmentAutoSolveEnabled);
        setAlignmentSolveLabel(i, AlignmentFlowCoordinator::waitingLabelText(), UiStatusLevel::Info);
    }
}
...
void DIMM::resetAlignmentRuntimeForStop()
{
    ...
    for (int i = 0; i < 2; ++i) {
        AlignmentRuntimeCoordinator::resetCameraForStop(&m_alignmentLastFrame[i],
                                                        &m_alignmentLastFrameId[i],
                                                        &m_alignmentSolveRuntime[i]);
        setAlignmentSolveLabel(i, AlignmentFlowCoordinator::stoppedLabelText(), UiStatusLevel::Muted);
    }
}
...
void DIMM::handleAlignmentFramePacket(int cameraIndex, const CameraFrame& packet)
{
    ...
    frameGate.lastPreviewMs = cameraIndex >= 0 && cameraIndex < 2
                                  ? m_alignmentLastPreviewMs[cameraIndex]
                                  : -1;
    ...
    auto& solveRuntime = m_alignmentSolveRuntime[cameraIndex];
...
bool DIMM::handleAutomaticAlignmentFrameTracking(int cameraIndex, const cv::Mat& frame, qint64 nowMs)
{
    ...
    double trackedPeak = 0.0;
    if (trackAlignmentPolarisLocally(cameraIndex, frame, &trackedPosition, &trackedPeak)) {
        AlignmentController::applyTrackingSuccess(&solveRuntime, trackedPosition);
        AlignmentController::syncTrackedPolarisPosition(cameraIndex,
                                                        trackedPosition,
                                                        runtime.confirmedPolarisPosition,
                                                        runtime.hasConfirmedPolarisPosition,
    ...
    if (AlignmentController::applyTrackingFailure(&solveRuntime,
                                                  lostTrackRetryCount,
                                                  nowMs,
...
bool DIMM::prepareAlignmentFramePreview(int cameraIndex, const CameraFrame& packet)
{
    ...
    m_alignmentLastFrame[cameraIndex] = packet.image.clone();
    m_alignmentLastFrameId[cameraIndex] = packet.frameId;
    return true;
}
...
void DIMM::requestAutomaticPolarisSolve(int cameraIndex, bool force)
{
    ...
    if (m_alignmentLastFrame[cameraIndex].empty()) {
    ...
    AlignmentTaskManager::submitFullSolve(m_polarisSolverController,
                                          cameraIndex,
                                          m_alignmentLastFrame[cameraIndex],
                                          solverConfig,
                                          m_alignmentSolveGeneration,
                                          m_alignmentLastFrameId[cameraIndex],
                                          force);
                                        ...
                                        double* peakValue)
{
    ...
    return AlignmentLocalTracker::trackFromConfirmedPosition(
        ...
        &m_alignmentSolveRuntime[cameraIndex],
        centroidDetector,
        trackedPosition,
        peakValue);
}
...
PolarisSolverConfig DIMM::buildPolarisSolverConfig() const
{
    ...
    config.hotPixelTemplates[0].maskPath = resolvePathFromAppDir(m_hotPixelCamera0MaskPath);
    config.hotPixelTemplates[0].excessPath = resolvePathFromAppDir(m_hotPixelCamera0ExcessPath);
    ...
    config.hotPixelTemplates[1].maskPath = resolvePathFromAppDir(m_hotPixelCamera1MaskPath);
    config.hotPixelTemplates[1].excessPath = resolvePathFromAppDir(m_hotPixelCamera1ExcessPath);
...
{
    if (result.generation != m_alignmentSolveGeneration ||
        result.cameraIndex < 0 ||
        result.cameraIndex >= 2 ||
        m_captureState != CaptureState::Alignment) {
        return;
    }

    auto& solveRuntime = m_alignmentSolveRuntime[result.cameraIndex];
    ...
    logPolarisSolveResult(result);
    m_alignmentSolveResult[result.cameraIndex] = result;
...
void DIMM::onPolarisSolveStatusChanged(int cameraIndex,
                                       ...
                                       quint64 generation)
{
    if (generation != m_alignmentSolveGeneration ||
        m_captureState != CaptureState::Alignment ||
        cameraIndex < 0 ||
        cameraIndex >= 2) {
        return;
    ...
    if (m_alignmentSolveRuntime[cameraIndex].state == AlignmentSolveState::ManualOnly) {
        return;
    }
...
{
    AlignmentCandidateController::CandidateCollectionInput input;
    ...
    input.lastFrameId = m_alignmentLastFrameId[cameraIndex];
    ...
    input.candidateDetector = [mono8](const cv::Mat& grayscale, double* peak) {
        if (mono8) {
            *mono8 = normalizeInitialStarDetectionFrame(grayscale);
            if (mono8->empty()) {
                return QVector<InitialStarCandidate>();
            }
            return detectInitialStarCandidates(*mono8, peak);
        }
        cv::Mat normalized = normalizeInitialStarDetectionFrame(grayscale);
        if (normalized.empty()) {
            return QVector<InitialStarCandidate>();
        }
        return detectInitialStarCandidates(normalized, peak);
    };
    return AlignmentCandidateController::collectCandidates(input, mono8, peakValue);
}
    ...
    QPointF* selectedStar)
{
    ...
    const bool canApplyAlignmentSelection =
        AlignmentManualSelectionController::canApplyCandidateSelection(
            manualSelectionRequested,
...
{
    auto& runtime = m_liveRuntime;
    AlignmentCandidateController::RuntimeAccess access;
    ...
    return AlignmentCandidateController::selectInitialCandidate(access,
                                                               candidates,
                                                               manualSelectionRequested,
                                                               preferredTarget);
}
    ...
    InitialStarSelection* selection)
{
    ...
    if (!AlignmentManualSelectionController::shouldShowCandidatePrompt(lastPromptMs, nowMs)) {
        return true;
    }
    ...
    if (!promptAlignmentCandidateSelection(cameraIndex, candidates, &chosenCandidateIndex)) {
        AlignmentManualSelectionController::recordCandidatePromptCancelled(
    ...
    AlignmentManualSelectionController::recordCandidatePromptAccepted(
        &runtime.selectedInitialCandidateIndex[cameraIndex],
        &runtime.lastInitialCandidatePromptMs[cameraIndex],
    ...
    QPointF* selectedStar)
{
    ...
    access.pendingInitialCandidateSelectionRequired =
        &runtime.pendingInitialCandidateSelectionRequired[cameraIndex];
    access.selectedInitialCandidateIndex = &runtime.selectedInitialCandidateIndex[cameraIndex];
    access.selectionRequested = &m_alignmentSelectionRequested[cameraIndex];
    AlignmentCandidateController::recordSelectedCandidate(access,
                                                          star,
    ...
    int* chosenCandidateIndex)
{
    ...
    const QStringList candidateLines =
        AlignmentManualSelectionController::candidatePromptLines(candidates);
    bool ok = false;
...
{
    auto& solveRuntime = m_alignmentSolveRuntime[cameraIndex];
    AlignmentManualSelectionController::applyManualConfirmation(&solveRuntime, star);
    if (m_polarisSolverController) {
        m_polarisSolverController->cancelCamera(cameraIndex, m_alignmentSolveGeneration);
    }
    const QString manualConfirmedMessage =
        AlignmentManualSelectionController::manualConfirmedMessage(star);
                                                      ...
                                                      double* peakValue)
{
    ...
    access.hasLastTargetPosition = &runtime.hasLastTargetPosition[cameraIndex];
    access.selectedInitialCandidateIndex = &runtime.selectedInitialCandidateIndex[cameraIndex];
    const AlignmentCandidateController::CentroidDetector centroidDetector =
    ...
    AlignmentCandidateController::updateFromFallbackCentroid(access,
                                                             frame,
...
void DIMM::updateAlignmentOverlay(int cameraIndex, const cv::Mat& frame)
{
    ...
    const PolarisSolveResult& solved = m_alignmentSolveResult[cameraIndex];
    auto& solveRuntime = m_alignmentSolveRuntime[cameraIndex];
    const bool hasCurrentSolverResult = solved.generation == m_alignmentSolveGeneration;
    ...
    const bool allowGuiCandidateDetection =
        !m_alignmentAutoSolveEnabled || m_alignmentSelectionRequested[cameraIndex];
    QVector<InitialStarCandidate> candidates =
