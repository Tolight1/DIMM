...
cv::Mat normalizeDetectionFrame(const cv::Mat& frame)
{
    ...
    if (maxValue > minValue) {
        grayscale.convertTo(mono8, CV_8UC1, 255.0 / (maxValue - minValue),
                                       ...
                                       const PolarisSolverConfig& config)
{
    ...
    if (!hot.enabled ||
        ...
        !QFileInfo::exists(hot.maskPath) ||
        !QFileInfo::exists(hot.excessPath)) {
        return frame;
    }
    ...
    const QByteArray maskBytes = maskFile.readAll();
    const QByteArray excessBytes = excessFile.readAll();
    if (maskBytes.size() < pixelCount ||
        excessBytes.size() < pixelCount * static_cast<qsizetype>(sizeof(quint16))) {
        return frame;
    }
                                           ...
                                           const PolarisSolverConfig& config)
{
    ...
    const cv::Mat grayscale = grayscaleDetectionFrame(correctedFrame);
    const cv::Mat mono8 = normalizeDetectionFrame(grayscale);
    if (mono8.empty()) {
        return detections;
    }
    ...
    QMetaObject::invokeMethod(
        ...
        [this, cameraIndex, taskId, frameCopy, config, generation, frameId, cancelled]() mutable {
            ...
            QMetaObject::invokeMethod(
                this,
                [this, result]() mutable {
                    if (result.cameraIndex >= 0 && result.cameraIndex < 2) {
