...
void AlignmentController::applyTrackingSuccess(AlignmentCameraSolveRuntime* runtime,
                                               const QPointF& trackedPosition)
{
    PolarisTracker::recordTrackSuccess(runtime, trackedPosition);
}
...
bool AlignmentController::applyTrackingFailure(AlignmentCameraSolveRuntime* runtime,
                                               int lostTrackRetryCount,
                                               qint64 nowMs,
                                               qint64 retryIntervalMs)
{
    return PolarisTracker::recordTrackFailure(runtime,
                                             lostTrackRetryCount,
                                             nowMs,
                                             retryIntervalMs);
}
