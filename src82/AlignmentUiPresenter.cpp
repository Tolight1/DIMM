...
QString AlignmentUiPresenter::waitingAlignmentLabelText()
{
    return QStringLiteral("自动识别: 等待全画幅");
}

QString AlignmentUiPresenter::stoppedAlignmentLabelText()
{
    return QStringLiteral("自动识别: 未启动");
}

QString AlignmentUiPresenter::startedAlignmentStatusText(double previewRateHz)
{
    return QStringLiteral("状态: 对准模式已启动，双相机 %1 Hz 全画幅预览")
        .arg(previewRateHz, 0, 'f', 1);
}

QString AlignmentUiPresenter::stoppedAlignmentStatusText()
{
