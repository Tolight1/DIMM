...
constexpr int kInitialFullFrameAverageFrames = 3;
constexpr int kLostCentroidRelocalizeFrames = 600;
...
bool detectInitialStarCentroid(const cv::Mat& grayscale, QPointF* centroid, double* peakValue)
{
    ...
    for (int y = 0; y < labels.rows; ++y) {
        ...
        for (int x = 0; x < labels.cols; ++x) {
            ...
            if (label > 0 && label < componentCount) {
                componentSignal[static_cast<size_t>(label)] += static_cast<double>(imageRow[x]);
            }
        }
    }
    ...
    for (int label = 1; label < componentCount; ++label) {
        ...
        const double signal = componentSignal[static_cast<size_t>(label)];
    ...
    *centroid = QPointF(centroids.at<double>(bestLabel, 0), centroids.at<double>(bestLabel, 1));
    return true;
}
    ...
    : QDialog(parent)
{
    ...
    opticsLayout->addWidget(new QLabel(QStringLiteral("焦距 f (cm):")), 2, 0);
...
bool SettingsDialog::applySettings()
{
    ...
    if (hotPixelEnabled) {
        ...
        for (int i = 0; i < hotPixelFiles.size(); ++i) {
            ...
            if (!QFileInfo::exists(hotPixelFiles[i])) {
                showInvalid(QStringLiteral("%1 文件不存在: %2").arg(hotPixelNames[i], hotPixelFiles[i]));
                return false;
            }
        }
    }
    ...
    , ui(new Ui_DIMM)
{
    ...
    m_imageProcessor->setTargetFrameRateHz(m_pulseGeneratorFrequencyHz);
    {
        ...
        if (!loadedThresholdPath.isEmpty()) {
            ...
            if (loadHotPixelTemplateSettings(loadedThresholdPath, &hotSettings)) {
                ...
                m_hotPixelCamera0MaskPath = hotSettings.camera0Mask;
                m_hotPixelCamera0ExcessPath = hotSettings.camera0Excess;
                m_hotPixelCamera1MaskPath = hotSettings.camera1Mask;
                m_hotPixelCamera1ExcessPath = hotSettings.camera1Excess;
    ...
    if (ui->lblCameraIndicator) {
        ui->lblCameraIndicator->hide();
    }
    ...
    m_settingsDialog->onApplyAutoExposure =
        [this](bool enabled,
            ...
            m_liveRuntime.lastAutoExposureFrame[0] = -1000;
            m_liveRuntime.lastAutoExposureFrame[1] = -1000;
            m_simulationRuntime.lastAutoExposureFrame[0] = -1000;
            m_simulationRuntime.lastAutoExposureFrame[1] = -1000;
            setStatusMessage(enabled ? QStringLiteral("自动曝光已启用") : QStringLiteral("自动曝光已关闭"),
    ...
    m_settingsDialog->onApplyHotPixelTemplates =
        [this](bool enabled,
            ...
            m_hotPixelCamera0MaskPath = enabled ? camera0MaskPath : QString();
            m_hotPixelCamera0ExcessPath = enabled ? camera0ExcessPath : QString();
            m_hotPixelCamera1MaskPath = enabled ? camera1MaskPath : QString();
            m_hotPixelCamera1ExcessPath = enabled ? camera1ExcessPath : QString();
            m_hotPixelTemplateWidth = enabled ? templateWidth : 0;
            m_hotPixelTemplateHeight = enabled ? templateHeight : 0;

            m_imageProcessor->configureHotPixelTemplates(m_hotPixelCamera0MaskPath,
                                                         m_hotPixelCamera0ExcessPath,
                                                         m_hotPixelCamera1MaskPath,
                                                         m_hotPixelCamera1ExcessPath,
                                                         m_hotPixelTemplateWidth,
                                                         m_hotPixelTemplateHeight);
            setStatusMessage(enabled ? QStringLiteral("热像素模板修正已启用")
                                     : QStringLiteral("热像素模板修正已关闭"),
                             enabled ? UiStatusLevel::Success : UiStatusLevel::Warning);
        };
...
void DIMM::refreshActionStates()
{
    ...
    const bool simulationActive = m_captureState == CaptureState::Simulation;
    const bool busy = m_connectingCameras;
    ui->btnStop->setEnabled(m_captureState != CaptureState::Idle && !busy);
    ui->btnCam1->setEnabled(hasAnyOpenCamera() || simulationActive);
    ui->btnCam2->setEnabled(hasAnyOpenCamera() || simulationActive);
...
{
    if (m_currentCamera < 0 || m_currentCamera > 1) {
        m_currentCamera = 0;
    }

    const bool simulationActive = m_captureState == CaptureState::Simulation;

    if (!simulationActive && m_cameraManager) {
        if (!m_cameraManager->isOpen(m_currentCamera) && m_cameraManager->isOpen(1 - m_currentCamera)) {
            m_currentCamera = 1 - m_currentCamera;
        }
    }

    if (ui->btnCam1 && ui->btnCam2 && ui->btnCam1->isVisible() && ui->btnCam2->isVisible()) {
        const QSignalBlocker blockCam1(ui->btnCam1);
        const QSignalBlocker blockCam2(ui->btnCam2);
        ui->btnCam1->setChecked(m_currentCamera == 0);
        ui->btnCam2->setChecked(m_currentCamera == 1);
    }
...
void DIMM::resetMeasurementState()
{
    ...
    if (m_imageProcessor) {
        for (int i = 0; i < 2; ++i) {
            m_imageProcessor->setCurrentRoi(i, m_imageProcessor->getCurrentRoi(i));
        }
    }
...
{
    return m_captureState == CaptureState::Live || m_captureState == CaptureState::Simulation;
}
...
bool DIMM::isCentroidNearCurrentRoiEdge(int cameraIndex) const
{
    ...
    return localX < kRoiEdgeUpdateMarginPx ||
           localY < kRoiEdgeUpdateMarginPx ||
           localX > static_cast<double>(roi.w - 1 - kRoiEdgeUpdateMarginPx) ||
           localY > static_cast<double>(roi.h - 1 - kRoiEdgeUpdateMarginPx);
}

bool DIMM::shouldUpdateRoiForEdge() const
{
    if (!hasValidCentroidsForRoiUpdate()) {
        return false;
    }
    return isCentroidNearCurrentRoiEdge(0) || isCentroidNearCurrentRoiEdge(1);
}
...
void DIMM::handleLiveRoiCentroidLoss(int cameraIndex)
{
    ...
    if (runtime.lostCentroidFrameCount[cameraIndex] < kLostCentroidRelocalizeFrames) {
        return;
    }
                                  ...
                                  bool requireCentered) const
{
    ...
    if (m_captureState == CaptureState::Live && m_liveHardwareRoiActive) {
        const RoiRect roi = m_imageProcessor ? m_imageProcessor->getCurrentRoi(cameraIndex) : RoiRect();
        ...
        if (localX < roiEdgeMargin ||
            localY < roiEdgeMargin ||
            localX > static_cast<double>(roi.w) - roiEdgeMargin ||
            localY > static_cast<double>(roi.h) - roiEdgeMargin) {
            return false;
        }
    }
...
void DIMM::applyRoiSummary(const RoiRect& roi)
{
    ...
    m_lblStatusROI->setText(QStringLiteral("当前ROI: (%1, %2) %3x%4")
                                .arg(roi.x)
                                .arg(roi.y)
                                .arg(roi.w)
                                .arg(roi.h));
...
bool DIMM::applyLiveHardwareRois(const RoiRect rois[2], QString* reason)
{
    if (!m_liveRoiCapabilitiesValid) {
        if (reason) {
...
bool DIMM::maybeSeedRoiFromFrame(int cameraIndex, const cv::Mat& frame)
{
    ...
    if (m_captureState == CaptureState::Live) {
        const bool inLocatePhase = m_liveStartupPhase != LiveStartupPhase::Tracking;
        const bool frameLooksLikeHardwareRoi =
            frame.cols <= kFixedRoiSize && frame.rows <= kFixedRoiSize;
        // Live ROI seeding is only valid during the full-frame localization stage. Once the
        // cameras have switched into hardware ROI mode, re-seeding from a 64x64 ROI frame would
        // collapse the absolute ROI back toward the local ROI coordinate system.
        if (!inLocatePhase || (m_liveHardwareRoiActive && frameLooksLikeHardwareRoi)) {
            return false;
        }
    }
    ...
    if (!detectInitialStarCentroid(mono8, &centroid, &peakValue) &&
        !detectInitialStarCentroidFast(mono8, &centroid, &peakValue)) {
        return false;
    }
    ...
    const RoiRect seeded = sanitizeRoi(
        RoiRect{qRound(centroid.x()) - kFixedRoiSize / 2,
                ...
                kFixedRoiSize},
        cameraIndex);
    runtime.initialFullFrameSum[cameraIndex].release();
    runtime.initialFullFrameCount[cameraIndex] = 0;
...
bool DIMM::commitPairedInitialRoisIfReady()
{
    ...
    QString reason;
    if (m_captureState == CaptureState::Live && !applyLiveHardwareRois(pairedRois, &reason)) {
        m_liveHardwareRoiActive = false;
        setStatusMessage(reason.isEmpty()
    ...
    m_imageProcessor->setCurrentRoi(0, pairedRois[0]);
    m_imageProcessor->setCurrentRoi(1, pairedRois[1]);
    ...
    m_liveStartupPhase = LiveStartupPhase::Tracking;
    applyRoiSummary(m_currentCamera == 0 ? pairedRois[0] : pairedRois[1]);
...
void DIMM::updateMinuteRoi(bool force)
{
    ...
    if (!force && !hasValidCentroidsForRoiUpdate()) {
        return;
    }
    ...
    RoiRect roi1;
    if (hasValidCentroidsForRoiUpdate()) {
        roi0 = buildCameraCentroidRoi(0);
        roi1 = buildCameraCentroidRoi(1);
    } else {
        const auto& runtime = activeRuntime();
        const QSize fallbackSize0 = runtime.frameSize[0].isValid() ? runtime.frameSize[0] : QSize(5120, 5120);
        const QSize fallbackSize1 = runtime.frameSize[1].isValid() ? runtime.frameSize[1] : QSize(5120, 5120);
        roi0 = sanitizeRoi(
            RoiRect{fallbackSize0.width() / 2 - kFixedRoiSize / 2,
                    fallbackSize0.height() / 2 - kFixedRoiSize / 2,
                    kFixedRoiSize,
                    kFixedRoiSize},
            0);
        roi1 = sanitizeRoi(
            RoiRect{fallbackSize1.width() / 2 - kFixedRoiSize / 2,
                    fallbackSize1.height() / 2 - kFixedRoiSize / 2,
                    kFixedRoiSize,
                    kFixedRoiSize},
            1);
    }

    m_imageProcessor->setCurrentRoi(0, roi0);
    m_imageProcessor->setCurrentRoi(1, roi1);
    applyRoiSummary(m_currentCamera == 0 ? roi0 : roi1);

    if (m_captureState == CaptureState::Live && hasValidCentroidsForRoiUpdate()) {
        QString reason;
        const RoiRect liveRois[2] = {roi0, roi1};
        if (applyLiveHardwareRois(liveRois, &reason)) {
...
void DIMM::onSwitchCamera1()
{
    ...
    syncCameraSelectionUi();
    if (m_captureState == CaptureState::Simulation) {
        runtimeForState(CaptureState::Simulation).lastSimulationPreviewFrame = -1;
        onUpdateSimulation();
    }
}
...
void DIMM::handleLiveFramePacket(int cameraIndex, const CameraFrame& packet)
{
    ...
    if (cameraIndex >= 0 && cameraIndex < 2) {
        ...
        if (shouldRefreshPreview) {
            if (targetCanvas) {
                // Once the camera switches into hardware ROI mode, the incoming live frame is only
                // 64x64. Keep the last full-frame image on screen and only update the ROI overlay.
                if (!(m_liveHardwareRoiActive && frameLooksLikeHardwareRoi)) {
...
void DIMM::on1hzTick()
{
    ...
    if (now.second() == 59 && minuteKey != runtime.lastRoiUpdateMinute && hasValidCentroidsForRoiUpdate()) {
        updateMinuteRoi();
        runtime.lastRoiUpdateMinute = minuteKey;
    }
}
...
void DIMM::applyAutoExposure(int cameraIndex, double peakValue)
{
    ...
    auto& runtime = activeRuntime();
    if (runtime.frameCount - runtime.lastAutoExposureFrame[cameraIndex] < m_autoExposureCooldownFrames) {
        return;
    }

    const bool tooDark = peakValue < m_autoExposureLowThreshold;
    const bool tooBright = peakValue > m_autoExposureHighThreshold;
    if (!tooDark && !tooBright) {
        return;
    }

    const double currentExposure = m_cameraManager->getExposure(cameraIndex);
    if (currentExposure <= 0.0) {
        return;
    }

    double targetExposure = currentExposure;
    if (tooDark) {
        targetExposure *= m_autoExposureDarkRatio;
    } else if (tooBright) {
        targetExposure *= m_autoExposureBrightRatio;
    }
    targetExposure = qBound(m_autoExposureMinUs, targetExposure, m_autoExposureMaxUs);

    if (qAbs(targetExposure - currentExposure) < 1.0) {
        return;
    }

    if (m_cameraManager->setExposure(cameraIndex, targetExposure)) {
        runtime.lastAutoExposureFrame[cameraIndex] = runtime.frameCount;
        setStatusMessage(QStringLiteral("自动曝光: 相机%1 %2 -> %3 μs (峰值 %4)")
                             .arg(cameraIndex + 1)
